...
// Symbol 是 JavaScript 语言的第七种数据类型，表示独一无二的值。
...
// 如果Symbol的参数是一个对象，就会调用该对象的toString方法，将其转为字符串，然后才生成一个 Symbol 值。
const obj = {
  toString() {
    return 'abc';
  }
...
# `Symbol`的API
``` bash
// Symbol.for() 接受一个字符串作为参数，然后搜索有没有以该参数作为描述的 Symbol 值。
// 如果有，就返回这个 Symbol 值，否则就新建并返回一个以该字符串为描述的 Symbol 值。
let s1 = Symbol.for('foo');
let s2 = Symbol.for('foo');
s1 === s2 // true

// Symbol.for()、Symbol()都会生成新的 Symbol。区别是，前者会被登记在全局环境中供搜索，后者不会
...
# 内置的 Symbol 值
``` bash
// ES6 提供了11个内置的Symbol值，指向语言内部使用的属性、方法、对象。
...
----------------------------------------------------------------------------
// Symbol.isConcatSpreadable 属性等于一个布尔值，
// 表示该对象用于Array.prototype.concat()时，是否可以展开。
// 数组的Symbol.isConcatSpreadable默认等于undefined，即可以展开。
// Symbol.isConcatSpreadable 属性等于true/undefined，都表示可展开。
let arr1 = ['c', 'd'];
...
// 类似数组的对象的Symbol.isConcatSpreadable属性默认为false，即不可展开。
...
------------------------------------------------------------------------------------------
// Symbol.species属性，指向一个构造函数。该构造函数用来创建衍生对象
...
// 调用then方法，创建衍生对象时，T1调用的是自身的构造方法，而T2调用的是Promise的构造方法。
...
// 方法会收到两个参数，第一个参数是replace方法正在作用的对象，下面例子是Hello，第二个参数是替换后的值，下面例子是World。
...
----------------------------------------------------------------------------------
// Symbol.toPrimitive 指向一个方法。该对象被转为原始类型的值时，会调用这个方法，返回该方法的返回值。
...
--------------------------------------------------------------------------------
// Symbol.toStringTag 用来定制对象上面调用toString方法时[object Object]或[object Array]中object后面的那个字符串。
// 例一
...
// 上面代码通过指定Symbol.unscopables属性，使得with语法块不会在当前作用域寻找foo属性，
// 即foo将指向外层作用域的变量。
```